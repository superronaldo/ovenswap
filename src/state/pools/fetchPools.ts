...
import { fetchPublicPoolsData } from './fetchPublicFarmData'
...
const fetchPools = async (farmsToFetch: SerializedFarmConfig[]): Promise<SerializedFarm[]> => {
  const [farmResult, masterChefResult] = await Promise.all([
    fetchPublicPoolsData(farmsToFetch),
