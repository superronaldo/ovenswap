...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://ovenswap.fi - check the URL carefully.")
    return warningText.split(/(https:\/\/ovenswap.fi)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://ovenswap.fi'}
          color={text === 'https://ovenswap.fi' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
